...
func GenerateToken(userId int) (string, error) {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"userId": userId,
		"exp":    time.Now().Add(time.Hour * 24 * 10000).Unix(),
	})
	configKey := beego.AppConfig.String("auth_key")
	return token.SignedString([]byte(configKey))
}
...
func (self *LoginController) doLogin(username, password string, explicit bool) bool {
	...
	if err == nil && b && !auth {
		file.GetDb().JsonDb.Clients.Range(func(key, value interface{}) bool {
			...
			if auth {
				token, err := GenerateToken(v.Id)
				self.SetSession("isAdmin", false)
				self.SetSession("userId", v.Id)
				self.SetSession("token", token)
				self.SetSession("err", err)
