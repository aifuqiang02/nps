(function ($) {
  ...
  function setchartlang(langobj, chartobj) {
    ...
    for (key in langobj) {
      var item = key;
      children = chartobj.hasOwnProperty(item)
        ? setchartlang(langobj[item], chartobj[item])
        : setchartlang(langobj[item], undefined);
    ...
    if (flag) {
      return {
        value:
          langobj[languages["current"]] ||
          langobj[languages["default"]] ||
          "N/A",
      };
    }
  }
  ...
  $.fn.setLang = function (dom) {
    ...
    if (dom == "") {
      $("#languagemenu span").text(
        " " + languages["menu"][languages["current"]]
      );
...
function langreply(langstr) {
  var langobj =
    languages["content"]["reply"][
      langstr.replace(/[\s,\.\?]*/g, "").toLowerCase()
    ];
  if ($.type(langobj) == "undefined") return langstr;
  langobj =
    langobj[languages["current"]] || langobj[languages["default"]] || langstr;
  return langobj;
}
  ...
  switch (action) {
    ...
    case "delete":
      ...
      action =
        langobj[languages["current"]] ||
        langobj[languages["default"]] ||
        "Are you sure you want to " + action + " it?";
